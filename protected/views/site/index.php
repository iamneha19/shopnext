...
<aside id="center_div">
	...
	?>
		<?php if($deals->getItemCount()>0 && $deals->getItemCount()>9) {?>
		<div id="load_more"><a href="javascript:void(0);">LOAD MORE DEALS</a></div>		
		<?php }  ?>
	<?php }  ?>
...
<script>
	$(document).ready(function(){
		...
		$('#center_div').on('click','.favourite.deal',function(){
			if(check_session=='')
			{
				$('#overlay').fadeIn();
				$('#login_main').fadeIn();
				...
				var content= $(this);
				var deal_id = $(this).attr('data-value');
				var user_id = $(this).attr('data-user'); 
				$.ajax({
					type:"POST",
					url:site_url+"/site/LikeStatus",
					dataType:'html',
					data:{deal_id:deal_id,user_id:user_id,type:'deal'},
					success: function(result){
						...
						if(obj[0]!='false')
						{
							...
							{
								content.siblings('.deal_like .like_text').html("Like");
							}
						}else{
							$('#overlay').fadeIn();
							$('#login_main').fadeIn();
	...
	$.ajax({
			...
			success:function(result)
			{
				if(result!='')
				{
					...
					$('#load_more').html('<a href="javascript:void(0);">LOAD MORE DEALS</a>').css('background','#1e3060');
					offset = parseInt(offset)+parseInt(limit);
				}else{
					$('#load_more').hide();
					$('.no_more_data').html("NO MORE DEALS TO DISPLAY....");
				}	
			},
			error: function(XMLHttpRequest, textStatus, errorThrown) { 
