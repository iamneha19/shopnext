<?php
	...
	/* get owner shop products id */
	$product_ids = Controller::getOwnerProductIds();
	$product_ids_string = implode(",",$product_ids);
	if(!empty($product_ids_string))
	{
		$list = CHtml::activeDropDownList($model,'product_id',CHtml::listData(Product::model()->findAll(array('condition'=>'status="1" and active_status="S" and product_id in ('.$product_ids_string.')','order'=>'name')), 'product_id', 'name'),array('prompt'=>'-- Products --'));
	}else{
		$list = false;
	}
?>
				...
				'columns'=>array(
						...
						array(
							...
							(
								'activate' => array
								(
									'url'=>'Yii::app()->createUrl("owner/productComment/setstatus",array("id"=>$data->comment_id,"active_status"=>"S"))',
									 'options'=>array(
											'id'=>'$data->comment_id',
								...
								'deactivate' => array
								(
									'url'=>'Yii::app()->createUrl("owner/productComment/setstatus",array("id"=>$data->comment_id,"active_status"=>"H"))',
									 'options'=>array(
											'id'=>'$data->comment_id',
