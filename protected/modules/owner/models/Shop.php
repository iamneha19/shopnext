...
class Shop extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('mark_invalid, active_status, status', 'length', 'max'=>1),
			array('invalid_remarks', 'length', 'max'=>500),
			array('description, address', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('shop_id, category_id, user_id, name, contact_no, description, address, locality_id, latitude, longitude, zip_code, shop_image_id, total_comments ,mark_invalid, invalid_remarks, active_status, added_on, updated_on, status', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search()
	{
		...
		if(empty($parent_id)){
			$criteria->AddCondition('`t`.status="1" and owner_id='.$owner_id);
		}else{
			$criteria->AddCondition('`t`.status="1" and owner_id='.$parent_id);
		}	
