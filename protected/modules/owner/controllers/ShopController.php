...
class ShopController extends Controller
{
	...
	/* public function actionCreate()
	{
		...
		if(isset($_POST['Shop']))
		{
			...
			if($model->locality_id!=''){
				$address  .= " ".$model->locality->locality;
			}if($model->city_id!=''){
				$address  .= " ".$model->city->city;
			}if($model->state_id!=''){
	...
	public function loadModel($id)
	{
		...
		if(empty($parent_id)){
			$condition = array("condition"=>"owner_id=".$owner_id);
		}else{
			$condition = array("condition"=>"owner_id=".$parent_id);
		}
		
		$model=Shop::model()->findByPk($id,$condition);
		if($model===null){
