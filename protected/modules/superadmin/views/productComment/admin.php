			...
			<?php $this->widget('bootstrap.widgets.TbGridView', array(
				...
				'columns'=>array(
						array(
							...
							'cssClassExpression' => '$data->product_id',
						),
						array(
							'header'=>'Sr. No.',
						...
						array(
							...
							'filter' =>CHtml::activeDropDownList($model,'product_id',CHtml::listData(Product::model()->findAll(array('condition'=>'status="1" and active_status="S"','order'=>'name')), 'product_id', 'name'),array('prompt'=>'-- Products --')),
						),
						// array(
							// 'name'=>'shop_id',
						...
						array(
							...
							(
								'activate' => array
								(
									'url'=>'Yii::app()->createUrl("superadmin/productComment/setstatus",array("id"=>$data->comment_id,"active_status"=>"S"))',
									 'options'=>array(
											'id'=>'$data->comment_id',
								...
								'deactivate' => array
								(
									'url'=>'Yii::app()->createUrl("superadmin/productComment/setstatus",array("id"=>$data->comment_id,"active_status"=>"H"))',
									 'options'=>array(
											'id'=>'$data->comment_id',
...
<script>
	product_ids = []; // global var to hold product ids whose comments are checked, and passed to changestatus action to update total comments
			...
			bootbox.confirm("Are you sure to perform this action?", function(result) {
				if(result)
				{
					...
					$.ajax({
						type:'POST',
						url:'changestatus',
						data:qry+'&status='+status+'&product_ids='+product_ids,
	...
	$('document').ready(function(){
		 ...
		 $("#content").on('click','input[name="comment_id_arr[]"]',function(){
			...
			var classes = $(this).parent().attr('class'); // 'checkbox-column {114}' product_id
			var classes = classes.split(" ");
			 
			if($(this).is(":checked")){ // if checked add in product_ids
				 if($.inArray( classes[1], product_ids ) == -1)
				 {
					product_ids.push(classes[1]); 
				 } 
