			...
			<?php $this->widget('bootstrap.widgets.TbGridView', array(
				...
				'columns'=>array(
						array(
							...
							'cssClassExpression' => '$data->deal_id',
						),
						array(
							'header'=>'Sr. No.',
						...
						array(
							'name'=>'deal_id',
							'value'=>'($data->deal_id!="") ? $data->deal->title:""',
							'filter' =>CHtml::activeDropDownList($model,'deal_id',CHtml::listData(Deal::model()->findAll(array('condition'=>'status="1" and active_status="S"','order'=>'title')), 'deal_id', 'title'),array('prompt'=>'-- Deal --')),
						),
						 array(
							'name'=>'user_id',
							'value'=>'($data->user_id != "") ? $data->user->name : ""',	
							'filter' =>CHtml::activeDropDownList($model,'user_id',CHtml::listData(User::model()->findAll(array('condition'=>'status="1" and active_status="S"','order'=>'name')), 'user_id', 'name'),array('prompt'=>'-- Username --')),
						), 	
						array(
							'name'=>'active_status',
						...
						array(
							...
							(
								'activate' => array
								(
									'url'=>'Yii::app()->createUrl("superadmin/dealComment/setstatus",array("id"=>$data->comment_id,"active_status"=>"S"))',
									 'options'=>array(
											'id'=>'$data->comment_id',
								...
								'deactivate' => array
								(
									'url'=>'Yii::app()->createUrl("superadmin/dealComment/setstatus",array("id"=>$data->comment_id,"active_status"=>"H"))',
									 'options'=>array(
											'id'=>'$data->comment_id',
...
<script>
	deal_ids = []; // global var to hold deal ids whose comments are checked, and passed to changestatus action to update total comments
			...
			bootbox.confirm("Are you sure to perform this action?", function(result) {
				if(result)
				{
					...
					$.ajax({
						type:'POST',
						url:'changestatus',
						data:qry+'&status='+status+'&deal_ids='+deal_ids,
	...
	$('document').ready(function(){
		 ...
		 $("#content").on('click','input[name="comment_id_arr[]"]',function(){
			...
			var classes = $(this).parent().attr('class'); // 'checkbox-column {114}' deal_id
			var classes = classes.split(" ");
			 
			if($(this).is(":checked")){ // if checked add in deal_ids
				 if($.inArray( classes[1], deal_ids ) == -1)
				 {
					deal_ids.push(classes[1]); 
				 } 
