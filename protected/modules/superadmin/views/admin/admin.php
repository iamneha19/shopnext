<?php
	...
	if(in_array('Create',$user_rights)){
		$this->menu=array(
			array('label'=>'Add Admin', 'url'=>array('create'),'htmlOptions' => array('class'=>'btn-default btn-sm'),'icon' => 'plus'),
		);
	}
	$template = "";
			...
			<?php $this->widget('bootstrap.widgets.TbGridView', array(
                                ...
                                'columns'=>array(
										array(
											...
											'selectableRows'=>2,
											'checkBoxHtmlOptions'=>array('name'=>'admin_id_arr[]'),
										),										 
                                        array(
											'header'=>'Sr. No.',
                                        ...
                                        array(
											...
											'filter' =>CHtml::activeDropDownList($model,'role_id',array_unique(CHtml::listData(Role::model()->findAll(array('condition'=>'status="1"','order'=>'name')), 'role_id', 'name')),array('prompt'=>'-- Role --')),
                                        ),
                                        'name',
                                        'email',
                                        'username',
                                        ...
                                        array(
											...
											'buttons'=>array( 
															...
															'activate' => array
															(
																'url'=>'Yii::app()->createUrl("superadmin/admin/activate",array("id"=>$data->admin_id))',
																	 'options'=>array(
																			'id'=>'$data->active_status',
																							'onClick'=>'return confirm("Are you sure to activate admin?")'
																		),
															...
															'deactivate' => array
															(
																'url'=>'Yii::app()->createUrl("superadmin/admin/deactivate",array("id"=>$data->admin_id))',
																	 'options'=>array(
																			'id'=>'$data->active_status',
																			'onClick'=>'return confirm("Are you sure to deactivate admin?")'
																		),
...
<script>
	...
	{
		var check = $("input[name='admin_id_arr[]']:checked");
