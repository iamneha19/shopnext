<?php
	...
	if(in_array('Update',$user_rights)){
		array_push($this->menu, array('label'=>'Update Brand', 'url'=>array('update', 'id'=>$model->brand_id),'htmlOptions' => array('class'=>'btn-default btn-sm'),'icon' => 'note'));
	}
	if(in_array('Create',$user_rights)){
		array_push($this->menu, array('label'=>'Delete Brand', 'url'=>'#','icon'=>'trash', 'htmlOptions'=>array('class'=>'btn-default btn-sm','submit'=>array('delete','id'=>$model->brand_id),'confirm'=>'Are you sure you want to delete this item?')));
	}
	if(in_array('Create',$user_rights)){
