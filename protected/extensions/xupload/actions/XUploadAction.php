...
class XUploadAction extends CAction {
		...
		{			
			...
			{				
				...
				else
				{
					if(!empty($_GET["id"]))
					{
						$path = $this->getPath()."/".$_GET["file"];
						$success = $this->deleteFile(array('path'=>$path));
						$this->deleteFileRecord($_GET['model'],$_GET['id']);
						$success = true;
					}	else	{
						$success = false;
    ...
    protected function handleUploading()
    {
        ...
        if ($model->{$this->fileAttribute} !== null) {
            ...
            $model->{$this->displayNameAttribute} = $_GET["id"]."_".time()."_".$model->{$this->fileAttribute}->getName();
            ...
            if ($model->validate()) {
				...
				$shop_image_id = $this->saveUploadedFile($_GET["model"],$_GET["id"],$model->{$this->displayNameAttribute});
     ...
     */
	protected function saveUploadedFile($model,$id,$name)
	{
		$return =  "";		
	...
	/**garima
     *Deletes uploaded file from db
     */
	protected function deleteFileRecord($model,$id) 
	{
		$return =  false;
		$model = strtoupper($model);
		switch ($model) {
			case 'SHOP':
					...
					$shop_model = new Shop;    
					$shop_model->updateAll(array('shop_image_id'=>null),array("condition"=>"shop_image_id=".$id));
					$return =  true;
				break;
			case 'PRODUCT':
					...
					$shop_model = new Product;    
					$shop_model->updateAll(array('product_image_id'=>null),array("condition"=>"product_image_id=".$id));
					$return =  true;
				break;
