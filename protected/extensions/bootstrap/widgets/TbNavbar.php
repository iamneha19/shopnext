...
class TbNavbar extends CWidget
{
	...
	*/
	...
	public function run()
	{
		...
		if ($this->brand !== false)
		{
			$this->brandOptions['style'] = 'padding: 0 !important;';
			echo CHtml::openTag('a', $this->brandOptions).CHtml::openTag('img', array('src'=>ApplicationConfig::app()->params['url_path']['applicationLogo'].'logo.png','alt'=>'logo')).'</a>';
			/*if (!empty(Yii::app()->params['logo']))
			{
				$this->brandOptions['style'] = 'padding: 0 !important;';
				echo CHtml::openTag('a', $this->brandOptions).CHtml::openTag('img', array('src'=>ApplicationConfig::app()->params['url_path']['applicationLogo'].Yii::app()->params['logo'],'alt'=>'logo')).'</a>';
			}
			else 
			{
