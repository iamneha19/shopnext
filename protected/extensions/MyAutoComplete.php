...
class myAutoComplete extends CJuiAutoComplete
{
    ...
    $attr_id = get_class($this->model).'_'.$this->attribute;
      ...
      echo CHtml::activeHiddenField($this->model, $this->attribute."_autocomplete");
        }else {
            // echo CHtml::textField($name,$this->value,$this->htmlOptions);       
    ...
    if (!isset($this->options['select'])) {
      $this->options['select'] = 'js:function(event, ui) {
            $("#'.$id.'").val(ui.item.label);
            $("#'.$attr_id.'_autocomplete").val(ui.item.id);
			
			var id = "'.$attr_id.'";
			
			if(id=="DailyDiary_client_case_id")
			{
				dynamicDate(ui.item.id);
			}
