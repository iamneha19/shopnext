...
class Country extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('country', 'match' ,'pattern'=>'/^[a-zA-Z ]*$/',
			'message'=> 'Country can contain only alphabets.'),
			array('added_on, updated_on', 'numerical'),
			array('country', 'length', 'max'=>50),
			array('country','unique','criteria'=>array('condition'=>'status="1"')),
			array('active_status, status', 'length', 'max'=>1),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('country_id, country, active_status, added_on, updated_on, status', 'safe', 'on'=>'search'),
		);
	}
