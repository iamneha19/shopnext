...
abstract class CActiveRecord extends CModel
{
	...
	protected function afterSave()
	{
		...
		$main_module = Yii::app()->controller->module;
		if(!empty($main_module)){
			$main_module = 	Yii::app()->controller->module->getId() ;
		}else{
			$main_module = '';
		}	
		
		if($main_module != 'api'){      // If API module is not called. 
			$module_name = $module = ucfirst(Yii::app()->controller->getId());
			$action_name = $operation = Yii::app()->controller->action->id;
			$success_msg = preg_replace('/(?<!^)([A-Z][a-z]|(?<=[a-z])[^a-z]|(?<=[A-Z])[0-9_])/', ' $1', $module);
			if($operation=='add')
			{
				$operation = ucfirst($operation)."ed";
			}
			else if($operation=='assignPermission')
			{
				$operation = 'Assign Permission';
			}
			...
			{
				$operation = ucfirst($operation)."d";
			}
			
			// switch case only used for to index solr data shop,city,locality,loglocation.
			$solr_actioins = array('create','update','setstatus');
			switch ($module_name) {
				case 'Shop':
						if(in_array($action_name,$solr_actioins)){
							$shop_data = Controller::getIndexData($this->shop_id);
				...
				case 'City':
						if(in_array($action_name,$solr_actioins)){
							$city_data = Controller::getCityIndexData($this->city_id);
				...
				case 'Locality':
						if(in_array($action_name,$solr_actioins)){
							$locality_data = Controller::getLocalityIndexData($this->locality_id);
							Yii::app()->locationSolr->updateOne($locality_data);
						}
				     break;
					 
				case 'Site':
						if($action_name=='logLocation'){ // To log the user location from frontend.
							$locality_data = Controller::getLocalionSearchIndexData($this->id);
							Yii::app()->locationSolr->updateOne($locality_data);
						}
				     break;
				default:
			...
			if($module!='' && $operation!='')
			{
				Yii::app()->user->setFlash('success', $success_msg." ".$operation." Successfully!");
			}
		}
